...
int main()
{
    ...
    {
        std::string user_text(buf);
        std::cout << "user text is {" << user_text << "}" << std::endl;
        TextMessage msg(user_text);
        ...
        if(sendto(socket_fd, msg.get().c_str(), readingSize, 0,
                (struct sockaddr*)&addr, sizeof(addr)) != readingSize)
        {
            error("write error");
            exit(EXIT_FAILURE);
        ...
        std::cout << "buf is [" << std::string(buf) << "]" << std::endl;
        ...
        if(receive_bytes < 0) error("receive error");
            ...
            std::cout << "data=[" << buf << "] " << sizeof(buf) << " bytes" << std::endl;
